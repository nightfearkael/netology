def solve(cook_book: list, person: int):
    ...
    for bludo in cook_book:
        spisok_bludo = f"{bludo[0]}: "
        product_list = []
        for product in bludo[1]:
            product_list.append(f"{product[0]} {product[1]*person} {product[2]}")
        spisok_bludo += ", ".join(product_list)
        result.append(spisok_bludo)
    return result
